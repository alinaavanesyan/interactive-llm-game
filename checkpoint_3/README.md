...
>
> **Валидационный датасет с размеченными ответами выбранной модели:** rag_validation_results.csv
>
> **Валидационный датасет с посчитанными метриками:** metrics.csv
...
Для улучшения качества retrieval и генерации в RAG-пайплайне мы использовали синтетическую аугментацию диалогов с помощью модели Gemini 3 Pro Preview.

*Почему выбран этот подход:*

**1. Семантическая релевантность к задаче:**
   - генерация диалогов с использованием оригинальных транскриптов BoJack Horseman позволяет создавать реплики, которые соответствуют стилю, эмоциональному состоянию и личности персонажей.
   - альтернативные методы аугментации** (например, простое переформулирование текстов или синонимизация) не сохраняют характер персонажей и контекст социальных взаимодействий, что критично для визуальной новеллы.

**2. Расширение покрытия редких или сложных сцен:** 
   - С помощью генерации можно создавать дополнительные варианты ответов для редких комбинаций вопросов и ситуаций, увеличивая вероятность того, что retrieval найдет релевантный контекст.
   - Это особенно важно для интерактивного сюжета, где игрок может задавать неожиданные вопросы или совершать нетипичные действия.

**3. Контроль качества контента:**
   - Gemini 3 Pro Preview имеет большой context window (до 1M токенов), что позволяет учитывать всю сцену или эпизод при генерации дополнительных реплик.
   - Это обеспечивает консистентность персонажей и сохранение ключевых деталей сюжета.

### Планы по дальнейшей аугментации:

Можно добавить транскрипты других сезонов BoJack Horseman (например, 2–3 сезона), чтобы увеличить разнообразие контекста и охватить больше сцен.

Ограничением является отсутствие готовых открытых источников для всех эпизодов. В таком случае может потребоваться парсинг аудиодорожки и последующая автоматическая транскрипция эпизодов.
...
**Главная метрика для дальнейшей работы** — `semantic similarity` между сгенерированным ответом и эталоном, поскольку:
...
2. **Проблемы с генерацией и оценкой качества генерации:**
   - Низкие F1 (0.12) отражает, что модель часто переформулирует или дополняет ответы, что не учитывается токенными метриками.  
